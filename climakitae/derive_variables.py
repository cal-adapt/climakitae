...
def _compute_wind_dir(u10, v10, name="wind_direction_derived"):
    ...
    Note: Plotting wind direction as a vectorfield overlay requires wind direction
            to be provided in radians. This is set in the plot_wind_velocity fn.
    """

    wind_dir = (
        270 - np.rad2deg(np.arctan2(-v10, -u10))
    ) % 360  # in radians, converted to degrees
    wind_dir.name = name
...
def _compute_wind_vel(u10, v10, plotting_unit="degrees", name="wind_velocity_derived"):
    ...
    ds_wind = xr.Dataset(
        {
            "wind_speed_derived": xr.DataArray(
                _compute_wind_mag(u10, v10),
                dims=("scenario", "simulation", "time", "y", "x"),
                coords={
                    "scenario": u10.scenario,
                    "time": u10.time,
            ...
            "wind_direction_derived": xr.DataArray(
                wind_dir,
                dims=("scenario", "simulation", "time", "y", "x"),
                coords={
                    "scenario": u10.scenario,
