        ...
        try:
            match x:
                case str() | datetime.date() | datetime.datetime():
                    ts = pd.to_datetime(x)
                    # Bare 4-digit year string (e.g. "2050"): snap to start/end
                    # of year, consistent with how integer years are handled.
                    if isinstance(x, str) and re.fullmatch(r"\d{4}", x.strip()):
                        if i == 0:
