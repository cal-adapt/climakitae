...
def write_gwl_files():
    """Call everything needed to write the global warming level reference files for all of the
    currently downscaled GCMs."""
    ...
    def build_timeseries(variable, model, ens_mem, scenarios):
        """Builds an xarray Dataset with only a time dimension, for the appended historical+ssp
        timeseries for all the scenarios of a particular model/variant combo. Works for all of the
        models(/GCMs) in the list models_for_now, which appear in the current data catalog of WRF
        downscaling."""
        scenario = "historical"
    ...
    def get_gwl(smoothed, degrees):
        """Takes a smoothed timeseries of global mean temperature for multiple scenarios,
        and returns a small table of the timestamp that a given global warming level
        is reached."""
    ...
    def get_gwl_table(
        ...
        """Loops through global warming levels, and returns an aggregate table for all warming
        levels (1.5, 2, 3, and 4 degrees) for all scenarios of the model/variant requested."""
