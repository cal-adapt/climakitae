...
The ClimateData interface has reached feature parity with the legacy `climakitae.core` interface for the catalogs that ship with `climakitae 1.5.x` (`cadcat`, `renewable energy generation`, `hdp`). All new features land here. See [Legacy API status](../legacy/index.md#status-and-roadmap) for the migration timeline.
