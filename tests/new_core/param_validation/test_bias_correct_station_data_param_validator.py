...
class TestValidateBiasCorrectStationDataParam:
    ...
    def setup_method(self):
        ...
        self.mock_station_metadata = pd.DataFrame({
            "station": ["Sacramento Executive Airport (KSAC)", "San Francisco (KSFO)"],
    ...
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator._get_station_metadata")
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator.find_station_match")
    def test_valid_minimal_parameters(self, mock_find_station, mock_get_metadata, valid_query):
    ...
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator._get_station_metadata")
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator.find_station_match")
    def test_valid_full_parameters(self, mock_find_station, mock_get_metadata, valid_query):
    ...
    def test_none_or_unset_value(self, value, valid_query):
        ...
        with pytest.warns(UserWarning, match="Station bias correction parameters cannot be None"):
            result = validate_bias_correction_station_data_param(value, query=valid_query)
    ...
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator._validate_stations")
    def test_invalid_station_validation_fails(self, mock_validate_stations, valid_query):
...
class TestValidateStations:
    ...
    @pytest.mark.parametrize("stations,error_match", [
...
class TestValidateHistoricalSlice:
    ...
    @pytest.mark.parametrize("historical_slice", [
        (1980, 2014),
        (1990, 2010),
        (1980, 2000),
        (2000, 2014),
    ...
    @pytest.mark.parametrize("historical_slice,error_match", [
...
class TestValidateWindow:
    ...
    @pytest.mark.parametrize("window,error_match", [
...
class TestValidateNquantiles:
    ...
    @pytest.mark.parametrize("nquantiles", [
        2, 10, 20, 30, 50
    ...
    @pytest.mark.parametrize("nquantiles,error_match", [
...
class TestValidateGroup:
    ...
    @pytest.mark.parametrize("group,error_match", [
...
class TestValidateKind:
    ...
    @pytest.mark.parametrize("kind,error_match", [
        (1, "must be a string"),
        ("-", "must be '\\+' .* or '\\*'"),
        ("/", "must be '\\+' .* or '\\*'"),
        ("", "must be '\\+' .* or '\\*'"),
...
class TestValidateVariableCompatibility:
    ...
    @pytest.mark.parametrize("variable_id", [
        "tas",
        "tasmax",
        "tasmin",
        "t2",
...
class TestValidateTimescaleRequirement:
    ...
    @pytest.mark.parametrize("table_id", ["day", "mon", "3hr"], ids=["daily", "monthly", "3hourly"])
...
class TestValidateInstitutionIdRequirement:
    ...
    @pytest.mark.parametrize("institution_id", [
        "CNRM",
        "DWD",
        None,
    ], ids=["cnrm", "dwd", "none"])
    def test_invalid_non_ucla_institution_ids(self, institution_id):
        ...
        with pytest.warns(UserWarning, match="requires 'institution_id' to be set to 'UCLA'"):
            result = _validate_institution_id_requirement(query)
        assert result is False
...
class TestValidateCatalogRequirement:
    ...
    @pytest.mark.parametrize("catalog,error_match", [
...
class TestGetStationMetadata:
    ...
    @patch("climakitae.new_core.param_validation.bias_correct_station_data_param_validator.DataCatalog")
    ...
    def test_contains_required_columns(self, mock_catalog_class):
        ...
        required_columns = ["station", "station id", "latitude", "longitude", "elevation"]
