...
class TestComputePairedDifference:
    ...
    def test_compute_paired_difference_missing_historic_column_uses_mean(
        ...
        the column in future_profile is not in historic_profile, triggering
        ...
        assert len(sim1_cols) == 2, "Should have 2 columns for sim1"
        assert all(
        ...
        assert len(sim2_cols) == 2, "Should have 2 colums for sim2"
        assert len(sim3_cols) == 2, "Should have 2 columns for sim3"
...
class TestCreateSingleWlMultiSimDataframe:
    ...
    def test_create_single_wl_multi_sim_dataframe_preserves_data_integrity(self):
        ...
        for i, sim in enumerate(test_simulations):
            ...
            profile_matrix = np.zeros(test_hours)
            for hour in range(test_hours):
                profile_matrix[hour] = (hour + 1) * 10
        ...
        for i, sim in enumerate(test_simulations):
            sim_key = f"test_{sim}_{i}"
            # reformat into list to compare with dataframe values
            expected_matrix = expected_values[sim_key]
            # Get column data for this simulation
        ...
        assert result[sim_a_key].loc[1] == 10.0, "Hour 0, Sim A should be 10"

        sim_b_key = "test_test_sim_B_1"
        assert result[sim_b_key].loc[24] == 240.0, "Hour 24, Sim B should be 240"
...
class TestCreateMultiWlMultiSimDataframe:
    ...
    def test_returns_dataframe(self):
        ...
        # Verify outcome: returns DataFrame with correct shape
        assert isinstance(result, pd.DataFrame), "Should return a pandas DataFrame"
        assert result.shape[0] == 8760, "Should have 365 rows (days)"

        # With 3 warming levels, 2 simulations: 3 * 2 = 6 columns
        expected_cols = len(self.warming_levels) * len(self.simulations)
