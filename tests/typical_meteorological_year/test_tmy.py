...
def test_compute_cdf(self):
    ...
    assert result[1][-1] == 1.0  # Max probability 1
...
def test_get_cdf_by_sim(self):
    ...
    assert result[0][0].max() == pytest.approx(
        test_data.isel(simulation=0).max(), abs=0.4
    )

    # Max of second simulation matches
    assert result[1][0].max() == pytest.approx(
        test_data.isel(simulation=1).max(), abs=0.4
    )
...
def test_get_cdf_by_mon_and_sim(self):
    ...
    assert result[1][0][0].max() == pytest.approx(
        test.isel({"simulation": 1}).groupby("time.month").max()[0], abs=0.4
    )
...
def test_get_cdf(self):
    ...
    assert result["temperature"].isel(simulation=0, month=6)[0].max() == pytest.approx(
...
def test_compute_weighted_fs(self):
    ...
    test = xr.DataArray(name="Daily max air temperature", data=test_data).to_dataset()
    vars_list = [
